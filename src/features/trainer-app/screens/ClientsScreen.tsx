...
const COLORS = ['#AAFF00', '#22D3EE', '#A78BFA', '#FACC15', '#F87171'];
...
function Skeleton({ h = 72 }: { h?: number }) {
  ...
  return <>{[0,1,2].map(k => <Animated.View key={k} style={{ height: h, borderRadius: 18, backgroundColor: '#1A1A1A', marginBottom: 10, opacity: op }} />)}</>;
}
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#0D0D0D', paddingTop: Platform.OS === 'android' ? (StatusBar.currentHeight ?? 24) : 0 }}>
      ...
      <View style={{ alignItems: 'center', paddingVertical: 12, paddingHorizontal: 20 }}>
        <View style={{ width: 68, height: 68, borderRadius: 34, backgroundColor: 'rgba(170,255,0,0.12)', borderWidth: 2, borderColor: '#AAFF00', alignItems: 'center', justifyContent: 'center', marginBottom: 10 }}>
          <Text style={{ color: '#AAFF00', fontSize: 22, fontWeight: '900' }}>{initials}</Text>
        </View>
        {profile && <Text style={{ color: '#AAAAAA', fontSize: 13 }}>{profile.email}</Text>}
        {profile?.status && (
          <View style={{ marginTop: 8, backgroundColor: 'rgba(170,255,0,0.10)', borderRadius: 999, paddingHorizontal: 12, paddingVertical: 4, borderWidth: 1, borderColor: 'rgba(170,255,0,0.25)' }}>
            <Text style={{ color: '#AAFF00', fontSize: 11, fontWeight: '700' }}>{profile.status}</Text>
          </View>
        )}
      </View>

      {/* Tab switcher */}
      <View style={{ flexDirection: 'row', marginHorizontal: 20, backgroundColor: '#1A1A1A', borderRadius: 16, padding: 4, borderWidth: 1, borderColor: '#222', marginBottom: 14 }}>
        {(['overview', 'timeline', 'notes'] as const).map(t => (
          <Pressable key={t} onPress={() => setTab(t)}
            style={{ flex: 1, paddingVertical: 9, borderRadius: 12, alignItems: 'center', backgroundColor: tab === t ? '#AAFF00' : 'transparent' }}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 48 }} showsVerticalScrollIndicator={false}>
        ...
        {tab === 'overview' && profile && (
          <View style={{ gap: 10 }}>
            {[
              ...
              <View key={row.label} style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#1A1A1A', borderRadius: 14, padding: 14, borderWidth: 1, borderColor: '#222' }}>
            ...
            {timeline.length === 0 ? (
              ...
              timeline.slice(0, 20).map((ev, i) => (
                <View key={i} style={{ flexDirection: 'row', alignItems: 'flex-start', marginBottom: 14 }}>
                  <View style={{ width: 30, height: 30, borderRadius: 9, backgroundColor: 'rgba(170,255,0,0.10)', borderWidth: 1, borderColor: 'rgba(170,255,0,0.22)', alignItems: 'center', justifyContent: 'center', marginRight: 12, marginTop: 2 }}>
                    <Feather name="zap" size={13} color="#AAFF00" />
                  </View>
                  <View style={{ flex: 1 }}>
        ...
        {tab === 'notes' && (
          <View>
            {/* Add note */}
            <TextInput
              style={{ backgroundColor: '#1A1A1A', borderRadius: 16, borderWidth: 1, borderColor: '#333', color: '#FFFFFF', padding: 14, fontSize: 14, height: 80, textAlignVertical: 'top', marginBottom: 10 }}
            ...
            <Pressable onPress={handleAddNote} disabled={adding || !noteText.trim()}
              style={{ backgroundColor: !noteText.trim() ? '#333' : '#AAFF00', borderRadius: 999, paddingVertical: 12, alignItems: 'center', marginBottom: 20, opacity: adding ? 0.7 : 1 }}>
            ...
            {notes.length === 0 ? (
              ...
              notes.map(n => (
                <View key={n.id} style={{ backgroundColor: '#1A1A1A', borderRadius: 16, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: '#222' }}>
        ...
        {!isLoading && (
          <View style={{ flexDirection: 'row', gap: 10, marginBottom: 14 }}>
            {[{ l: 'Total', v: clients.length }, { l: 'Active', v: clients.filter(c => c.status === 'Active').length }, { l: 'Shown', v: filtered.length }].map(s => (
              <View key={s.l} style={{ flex: 1, backgroundColor: '#1A1A1A', borderRadius: 16, padding: 12, alignItems: 'center', gap: 4, borderWidth: 1, borderColor: '#222' }}>
                <Text style={{ color: '#AAFF00', fontSize: 18, fontWeight: '900' }}>{s.v}</Text>
                <Text style={{ color: '#AAAAAA', fontSize: 10, fontWeight: '600' }}>{s.l}</Text>
              </View>
            ))}
          </View>
        )}

        <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#1A1A1A', borderRadius: 14, borderWidth: 1, borderColor: '#333', paddingHorizontal: 12, height: 46 }}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 48 }} showsVerticalScrollIndicator={false}>
        ...
        {isError && (
          <View style={{ alignItems: 'center', paddingTop: 60 }}>
            ...
            <Pressable onPress={() => refetch()} style={{ backgroundColor: '#AAFF00', borderRadius: 999, paddingHorizontal: 28, paddingVertical: 12, marginTop: 16 }}>
        ...
        {!isLoading && !isError && filtered.length === 0 && (
          <View style={{ alignItems: 'center', paddingTop: 60 }}>
            <View style={{ width: 68, height: 68, borderRadius: 34, backgroundColor: '#1A1A1A', borderWidth: 1, borderColor: '#222', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}>
          ...
          return (
            <Pressable key={c.assignmentId} onPress={() => setSelectedClient(c.clientId)}
              ...
              <View style={{ width: 46, height: 46, borderRadius: 23, backgroundColor: col + '18', borderWidth: 1.5, borderColor: col + '50', alignItems: 'center', justifyContent: 'center', marginRight: 14 }}>
                <Text style={{ color: col, fontSize: 14, fontWeight: '900' }}>#{c.clientId}</Text>
              </View>
              <View style={{ flex: 1 }}>
              ...
              <View style={{ alignItems: 'flex-end', gap: 6 }}>
                <View style={{ backgroundColor: c.status === 'Active' ? 'rgba(170,255,0,0.12)' : '#222', borderRadius: 999, paddingHorizontal: 10, paddingVertical: 3, borderWidth: 1, borderColor: c.status === 'Active' ? 'rgba(170,255,0,0.28)' : '#333' }}>
                  <Text style={{ color: c.status === 'Active' ? '#AAFF00' : '#AAAAAA', fontSize: 10, fontWeight: '700' }}>{c.status}</Text>
                </View>
