      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 60 }}>
        ...
        <View style={{ alignItems: 'center', paddingTop: 30, paddingBottom: 20, paddingHorizontal: 20 }}>
          <View style={{ width: 80, height: 80, borderRadius: 40, backgroundColor: 'rgba(170,255,0,0.12)', borderWidth: 2, borderColor: '#AAFF00', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>
            <Text style={{ color: '#AAFF00', fontSize: 26, fontWeight: '900' }}>{initials}</Text>
          </View>
          ...
          {profile?.trainerCode && (
            <View style={{ marginTop: 10, backgroundColor: '#1A1A1A', borderRadius: 999, paddingHorizontal: 12, paddingVertical: 4, borderWidth: 1, borderColor: '#333' }}>
          ...
          {!!profile?.specializations?.length && (
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 10, justifyContent: 'center' }}>
              {profile.specializations.map(s => (
                <View key={s} style={{ backgroundColor: 'rgba(170,255,0,0.10)', borderRadius: 999, paddingHorizontal: 10, paddingVertical: 3, borderWidth: 1, borderColor: 'rgba(170,255,0,0.25)' }}>
                  <Text style={{ color: '#AAFF00', fontSize: 11, fontWeight: '600' }}>{s}</Text>
                </View>
              ))}
            </View>
          )}
        ...
        {/* Tab switcher */}
        <View style={{ flexDirection: 'row', marginHorizontal: 20, backgroundColor: '#1A1A1A', borderRadius: 16, padding: 4, borderWidth: 1, borderColor: '#222', marginBottom: 20 }}>
          {(['info', 'schedule', 'earnings'] as const).map(t => (
            <Pressable key={t} onPress={() => setTab(t)}
              style={{ flex: 1, paddingVertical: 10, borderRadius: 12, alignItems: 'center', backgroundColor: tab === t ? '#AAFF00' : 'transparent' }}>
          ...
          <View style={{ paddingHorizontal: 20 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              ...
              <Pressable onPress={() => editing ? handleSaveProfile() : setEditing(true)} disabled={savingProfile}
                style={{ flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: editing ? '#AAFF00' : '#1A1A1A', borderRadius: 999, paddingHorizontal: 14, paddingVertical: 8, borderWidth: 1, borderColor: editing ? '#AAFF00' : '#333' }}>
            ...
            {[
              ...
              <View key={row.label} style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#1A1A1A', borderRadius: 14, padding: 14, marginBottom: 8, borderWidth: 1, borderColor: '#222' }}>
            ...
            <TextInput
              style={{ backgroundColor: editing ? '#222' : '#1A1A1A', borderRadius: 14, borderWidth: 1, borderColor: editing ? '#444' : '#222', color: editing ? '#FFFFFF' : '#888', padding: 14, fontSize: 14, height: 80, textAlignVertical: 'top', marginBottom: 12 }}
            ...
            <TextInput
              style={{ backgroundColor: editing ? '#222' : '#1A1A1A', borderRadius: 14, borderWidth: 1, borderColor: editing ? '#444' : '#222', color: editing ? '#FFFFFF' : '#888', padding: 14, fontSize: 14, marginBottom: 12 }}
              value={phone} onChangeText={setPhone} placeholder="+91…" placeholderTextColor="#555"
              keyboardType="phone-pad" editable={editing}
            />

            {/* Available toggle */}
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#1A1A1A', borderRadius: 14, padding: 14, borderWidth: 1, borderColor: '#222', marginBottom: 20 }}>
              <View>
              ...
              <Switch value={available} onValueChange={setAvailable} disabled={!editing}
                trackColor={{ false: '#333', true: '#AAFF00' }} thumbColor={available ? '#000' : '#FFFFFF'} />
            </View>
          ...
          <View style={{ paddingHorizontal: 20 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              ...
              <Pressable onPress={handleSaveSchedule} disabled={savingSchedule}
                style={{ backgroundColor: '#AAFF00', borderRadius: 999, paddingHorizontal: 14, paddingVertical: 8, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            ...
            {DAYS.map((day, i) => {
              ...
              return (
                <View key={day} style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: active ? 'rgba(170,255,0,0.07)' : '#1A1A1A', borderRadius: 14, padding: 14, marginBottom: 8, borderWidth: 1, borderColor: active ? 'rgba(170,255,0,0.22)' : '#222' }}>
                  ...
                  <Switch value={active} onValueChange={() => toggleSlot(dow)}
                    trackColor={{ false: '#333', true: '#AAFF00' }} thumbColor={active ? '#000' : '#FFFFFF'} />
                </View>
              );
            })}
          </View>
        ...
        {tab === 'earnings' && (
          <View style={{ paddingHorizontal: 20 }}>
            ...
            {earnings ? (
              <>
                <View style={{ backgroundColor: '#1A1A1A', borderRadius: 20, padding: 20, borderWidth: 1, borderColor: '#222', marginBottom: 16 }}>
                  ...
                  <Text style={{ color: '#FFFFFF', fontSize: 34, fontWeight: '900', letterSpacing: -1, marginBottom: 10 }}>
                ...
                {perf && (
                  <View style={{ backgroundColor: '#1A1A1A', borderRadius: 18, padding: 16, borderWidth: 1, borderColor: '#222', gap: 10 }}>
                    {[
