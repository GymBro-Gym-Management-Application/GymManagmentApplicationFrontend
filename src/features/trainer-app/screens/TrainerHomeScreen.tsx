...
function StatCard({ label, value, color, icon }: { label: string; value: string | number; color: string; icon: FeatherIconName }) {
  return (
    <View style={{ flex: 1, backgroundColor: '#1A1A1A', borderRadius: 18, padding: 14, borderWidth: 1, borderColor: '#222', alignItems: 'center', gap: 6 }}>
      ...
      <Text style={{ color: '#FFFFFF', fontSize: 20, fontWeight: '900', letterSpacing: -0.5 }}>{value}</Text>
      <Text style={{ color: '#AAAAAA', fontSize: 10, fontWeight: '600' }}>{label}</Text>
    </View>
  );
}
...
export default function TrainerHomeScreen({ trainerId, onNavigate }: Props) {
  ...
  const quickLinks: { icon: FeatherIconName; label: string; screen: string; color: string }[] = [
    ...
    { icon: 'activity',  label: 'Programs', screen: 'builder',  color: '#AAFF00' },
    ...
    <View style={{ flex: 1, backgroundColor: '#0D0D0D', paddingTop: Platform.OS === 'android' ? (StatusBar.currentHeight ?? 24) : 0 }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 110 }}>
        ...
        <ImageBackground source={IMG_HERO} style={{ width: '100%', height: 260 }} resizeMode="cover">
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.62)', padding: 20, justifyContent: 'space-between' }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingTop: 8 }}>
              <View style={{ width: 42, height: 42, borderRadius: 21, backgroundColor: 'rgba(170,255,0,0.18)', borderWidth: 2, borderColor: '#AAFF00', alignItems: 'center', justifyContent: 'center' }}>
                <Text style={{ color: '#AAFF00', fontSize: 16, fontWeight: '900' }}>{name[0]?.toUpperCase() ?? 'T'}</Text>
              </View>
            ...
            <View>
              ...
              <Text style={{ color: '#FFFFFF', fontSize: 28, fontWeight: '800', letterSpacing: -0.5, lineHeight: 34 }}>
                Your clients{'\n'}need <Text style={{ color: '#AAFF00' }}>you today</Text>
              </Text>
            </View>
          </View>
        </ImageBackground>
        ...
        <View style={{ flexDirection: 'row', paddingHorizontal: 20, gap: 10, marginTop: 20, marginBottom: 28 }}>
          <StatCard label="Clients"  value={perf?.totalClients  ?? clients.length} color="#AAFF00" icon="users" />
        ...
        {/* ── Earnings card ── */}
        {earnings && (
          <View style={{ marginHorizontal: 20, marginBottom: 28, backgroundColor: '#1A1A1A', borderRadius: 20, padding: 18, borderWidth: 1, borderColor: '#222' }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              ...
              <View style={{ backgroundColor: 'rgba(170,255,0,0.10)', borderRadius: 999, paddingHorizontal: 10, paddingVertical: 4, borderWidth: 1, borderColor: 'rgba(170,255,0,0.25)' }}>
                <Text style={{ color: '#AAFF00', fontSize: 10, fontWeight: '700' }}>Earnings</Text>
              </View>
            </View>
          ...
          {quickLinks.map(q => (
            <Pressable key={q.screen} onPress={() => onNavigate(q.screen)}
              style={({ pressed }) => ({
                ...
                backgroundColor: pressed ? q.color + '14' : '#1A1A1A',
                borderRadius: 18, borderWidth: 1,
                borderColor: pressed ? q.color + '35' : '#222',
              })}>
        ...
        {clients.length > 0 && (
          <View style={{ paddingHorizontal: 20 }}>
            ...
            {clients.slice(0, 4).map((c, i) => {
              const colors = ['#AAFF00', '#22D3EE', '#A78BFA', '#FACC15'];
              ...
              return (
                <Pressable key={c.assignmentId} onPress={() => onNavigate(`client-${c.clientId}`)}
                  ...
                  <View style={{ width: 42, height: 42, borderRadius: 21, backgroundColor: col + '18', borderWidth: 1.5, borderColor: col + '50', alignItems: 'center', justifyContent: 'center', marginRight: 14 }}>
                    <Text style={{ color: col, fontSize: 16, fontWeight: '900' }}>{c.clientId}</Text>
                  </View>
                  <View style={{ flex: 1 }}>
                  ...
                  <View style={{ backgroundColor: c.status === 'Active' ? 'rgba(170,255,0,0.12)' : '#222', borderRadius: 999, paddingHorizontal: 10, paddingVertical: 4, borderWidth: 1, borderColor: c.status === 'Active' ? 'rgba(170,255,0,0.28)' : '#333' }}>
        ...
        <View style={{ marginHorizontal: 20, marginTop: 8, backgroundColor: '#1A1A1A', borderRadius: 20, padding: 20, borderWidth: 1, borderColor: '#222', flexDirection: 'row', alignItems: 'center', gap: 14 }}>
          ...
          <View style={{ backgroundColor: '#222', borderRadius: 999, paddingHorizontal: 10, paddingVertical: 4, borderWidth: 1, borderColor: '#333' }}>
            <Text style={{ color: '#666', fontSize: 10, fontWeight: '700' }}>SOON</Text>
          </View>
        </View>
