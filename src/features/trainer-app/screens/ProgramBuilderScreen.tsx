...
function Skeleton({ count = 3, h = 90 }: { count?: number; h?: number }) {
  ...
  return <>{Array.from({ length: count }).map((_, k) => <Animated.View key={k} style={{ height: h, borderRadius: 18, backgroundColor: '#1A1A1A', marginBottom: 10, opacity: op }} />)}</>;
}

function ComingSoonBanner({ feature }: { feature: string }) {
  return (
    <View style={{ backgroundColor: '#1A1A1A', borderRadius: 18, padding: 20, borderWidth: 1, borderColor: '#222', alignItems: 'center', gap: 10, marginBottom: 14 }}>
      ...
      <View style={{ backgroundColor: '#222', borderRadius: 999, paddingHorizontal: 12, paddingVertical: 4, borderWidth: 1, borderColor: '#333' }}>
...
export default function ProgramBuilderScreen({ trainerId, onBack }: Props) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#0D0D0D', paddingTop: Platform.OS === 'android' ? (StatusBar.currentHeight ?? 24) : 0 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
          {onBack && (
            <Pressable onPress={onBack} hitSlop={12}
              style={{ width: 36, height: 36, borderRadius: 18, backgroundColor: '#1A1A1A', borderWidth: 1, borderColor: '#333', alignItems: 'center', justifyContent: 'center', marginRight: 14 }}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
          {(['exercises', 'workouts', 'plans', 'ai'] as Tab[]).map(t => (
            <Pressable key={t} onPress={() => setTab(t)}
              style={{ paddingHorizontal: 16, paddingVertical: 9, borderRadius: 999, backgroundColor: tab === t ? '#AAFF00' : '#1A1A1A', borderWidth: 1, borderColor: tab === t ? '#AAFF00' : '#333' }}>
        ...
        <>
              ...
              {EX_FILTERS.map(f => {
                ...
                return (
                  <Pressable key={f} onPress={() => { setExFilter(f); setExPage(1); }}
                    style={{ paddingHorizontal: 14, paddingVertical: 8, borderRadius: 999, backgroundColor: a ? '#AAFF00' : '#1A1A1A', borderWidth: 1, borderColor: a ? '#AAFF00' : '#333' }}>
                    <Text style={{ color: a ? '#000' : '#AAAAAA', fontSize: 12, fontWeight: '700' }}>{f}</Text>
                  </Pressable>
                );
              })}
            </ScrollView>
            <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#1A1A1A', borderRadius: 14, borderWidth: 1, borderColor: '#333', paddingHorizontal: 12, height: 44 }}>
              ...
              {exSearch.length > 0 && <Pressable onPress={() => setExSearch('')} hitSlop={8}><Feather name="x" size={13} color="#555" /></Pressable>}
            </View>
          </View>
          <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 48 }} showsVerticalScrollIndicator={false}>
            ...
            {!exLoad && filteredEx.map(ex => {
              ...
              return (
                <View key={ex.id} style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#1A1A1A', borderRadius: 18, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: '#222' }}>
                  <View style={{ width: 60, height: 60, borderRadius: 12, backgroundColor: '#222', borderWidth: 1, borderColor: '#333', alignItems: 'center', justifyContent: 'center', marginRight: 14 }}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <View style={{ flexDirection: 'row', gap: 6, marginTop: 6 }}>
                      {ex.category && <View style={{ backgroundColor: '#222', borderRadius: 999, paddingHorizontal: 8, paddingVertical: 2, borderWidth: 1, borderColor: '#333' }}><Text style={{ color: '#AAAAAA', fontSize: 10 }}>{ex.category}</Text></View>}
                      {ex.difficulty && <View style={{ backgroundColor: color + '15', borderRadius: 999, paddingHorizontal: 8, paddingVertical: 2, borderWidth: 1, borderColor: color + '35' }}><Text style={{ color, fontSize: 10, fontWeight: '700' }}>{ex.difficulty}</Text></View>}
                    </View>
                  </View>
                  <View style={{ width: 34, height: 34, borderRadius: 10, backgroundColor: '#AAFF00', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {tab === 'workouts' && (
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 48 }} showsVerticalScrollIndicator={false}>
          ...
          <Pressable onPress={() => setShowWForm(v => !v)}
            style={{ flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: '#AAFF00', borderRadius: 999, paddingHorizontal: 18, paddingVertical: 12, marginBottom: 14, alignSelf: 'flex-start' }}>
            <Feather name={showWForm ? 'chevron-up' : 'plus'} size={16} color="#000" />
            <Text style={{ color: '#000', fontSize: 13, fontWeight: '700' }}>New Workout</Text>
          </Pressable>

          {showWForm && (
            <View style={{ backgroundColor: '#1A1A1A', borderRadius: 20, padding: 16, borderWidth: 1, borderColor: '#222', marginBottom: 14 }}>
              <TextInput style={{ backgroundColor: '#222', borderRadius: 12, borderWidth: 1, borderColor: '#333', color: '#FFF', padding: 12, fontSize: 14, marginBottom: 10 }}
              ...
              <View style={{ flexDirection: 'row', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
                {GOALS.map(g => <Pressable key={g} onPress={() => setWGoal(g)}
                  style={{ paddingHorizontal: 12, paddingVertical: 6, borderRadius: 999, backgroundColor: wGoal === g ? '#AAFF00' : '#222', borderWidth: 1, borderColor: wGoal === g ? '#AAFF00' : '#333' }}>
              ...
              <View style={{ flexDirection: 'row', gap: 8, marginBottom: 10 }}>
                {DIFFS.map(d => <Pressable key={d} onPress={() => setWDiff(d)}
                  style={{ flex: 1, paddingVertical: 8, borderRadius: 12, alignItems: 'center', backgroundColor: wDiff === d ? 'rgba(170,255,0,0.12)' : '#222', borderWidth: 1, borderColor: wDiff === d ? 'rgba(170,255,0,0.35)' : '#333' }}>
                  <Text style={{ color: wDiff === d ? '#AAFF00' : '#AAA', fontSize: 11, fontWeight: '700' }}>{d}</Text>
                </Pressable>)}
              </View>
              <TextInput style={{ backgroundColor: '#222', borderRadius: 12, borderWidth: 1, borderColor: '#333', color: '#FFF', padding: 12, fontSize: 14, marginBottom: 12 }}
              ...
              <Pressable onPress={handleCreateWorkout} disabled={creating || !wName.trim()}
                style={{ backgroundColor: !wName.trim() ? '#333' : '#AAFF00', borderRadius: 999, paddingVertical: 12, alignItems: 'center', opacity: creating ? 0.7 : 1 }}>
          ...
          {!wLoad && workouts.map(w => {
            ...
            return (
              <View key={w.id} style={{ backgroundColor: '#1A1A1A', borderRadius: 18, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: '#222' }}>
                <Text style={{ color: '#FFFFFF', fontSize: 15, fontWeight: '700', marginBottom: 8 }}>{w.name}</Text>
                <View style={{ flexDirection: 'row', gap: 6 }}>
                  {w.goal && <View style={{ backgroundColor: gc + '15', borderRadius: 999, paddingHorizontal: 8, paddingVertical: 2, borderWidth: 1, borderColor: gc + '35' }}><Text style={{ color: gc, fontSize: 10, fontWeight: '700' }}>{w.goal}</Text></View>}
                  {w.difficulty && <View style={{ backgroundColor: '#222', borderRadius: 999, paddingHorizontal: 8, paddingVertical: 2, borderWidth: 1, borderColor: '#333' }}><Text style={{ color: '#AAAAAA', fontSize: 10 }}>{w.difficulty}</Text></View>}
                  {w.durationMin && <View style={{ backgroundColor: '#222', borderRadius: 999, paddingHorizontal: 8, paddingVertical: 2, borderWidth: 1, borderColor: '#333', flexDirection: 'row', alignItems: 'center', gap: 4 }}><Feather name="clock" size={9} color="#777" /><Text style={{ color: '#AAA', fontSize: 10 }}>{w.durationMin}m</Text></View>}
                </View>
              </View>
            );
          })}
      ...
      {tab === 'plans' && (
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 48 }} showsVerticalScrollIndicator={false}>
            ...
            <View style={{ backgroundColor: '#1A1A1A', borderRadius: 20, padding: 16, borderWidth: 1, borderColor: '#222', marginBottom: 14 }}>
              ...
              <View style={{ flexDirection: 'row', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
                {GOALS.map(g => <Pressable key={g} onPress={() => setPGoal(g)}
                  style={{ paddingHorizontal: 12, paddingVertical: 6, borderRadius: 999, backgroundColor: pGoal === g ? '#AAFF00' : '#222', borderWidth: 1, borderColor: pGoal === g ? '#AAFF00' : '#333' }}>
              ...
              <Pressable onPress={handleCreatePlan} disabled={creatingPlan || !pName.trim() || !pWeeks}
                style={{ backgroundColor: (!pName.trim() || !pWeeks) ? '#333' : '#AAFF00', borderRadius: 999, paddingVertical: 12, alignItems: 'center', opacity: creatingPlan ? 0.7 : 1 }}>
          ...
          {!pLoad && plans.map(plan => {
            ...
            return (
              <View key={plan.id} style={{ backgroundColor: '#1A1A1A', borderRadius: 18, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: '#222' }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                  ...
                  <View style={{ backgroundColor: 'rgba(170,255,0,0.10)', borderRadius: 10, paddingHorizontal: 10, paddingVertical: 6, alignItems: 'center', borderWidth: 1, borderColor: 'rgba(170,255,0,0.25)' }}>
                    <Text style={{ color: '#AAFF00', fontSize: 16, fontWeight: '900' }}>{plan.durationWeeks}</Text>
                    <Text style={{ color: '#AAA', fontSize: 8 }}>WKS</Text>
                  </View>
                </View>
              </View>
            );
