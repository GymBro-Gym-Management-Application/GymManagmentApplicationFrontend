...
function Skeleton({ height = 88, count = 4 }: { height?: number; count?: number }) {
  ...
  return <>{Array.from({ length: count }).map((_, k) =>
    <Animated.View key={k} style={{ height, borderRadius: 18, backgroundColor: '#1A1A1A', marginBottom: 10, opacity: op }} />
  )}</>;
}
  ...
  return (
    <View className="flex-1 bg-bg" style={{ paddingTop: Platform.OS === 'android' ? (StatusBar.currentHeight ?? 24) : 0 }}>
      ...
      <View className="px-5 pt-5 pb-3">
        ...
        {/* Tab switcher */}
        <View style={{ flexDirection: 'row', backgroundColor: '#1A1A1A', borderRadius: 16, padding: 4, borderWidth: 1, borderColor: '#222' }}>
          {(['workouts', 'exercises'] as const).map(t => (
            <Pressable key={t} onPress={() => setTab(t)}
              style={{ flex: 1, paddingVertical: 10, borderRadius: 12, alignItems: 'center',
                backgroundColor: tab === t ? '#AAFF00' : 'transparent' }}>
      ...
      {tab === 'workouts' && (
        <ScrollView className="flex-1 px-5" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 48 }}>
          ...
          {!wLoad && !wErr && workouts.map(w => {
            ...
            const dc = DIFF_COLOR[w.difficulty?.toLowerCase() ?? 'beginner'] ?? '#AAAAAA';
            return (
              <Pressable key={w.id} onPress={() => onOpenWorkout(w.id)}
                ...
                <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                  ...
                  {/* Start button */}
                  <View style={{ width: 36, height: 36, borderRadius: 18, backgroundColor: '#AAFF00', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <View style={{ flexDirection: 'row', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
                  ...
                  {w.difficulty && <View style={{ backgroundColor: dc + '15', borderRadius: 999, paddingHorizontal: 8, paddingVertical: 2, borderWidth: 1, borderColor: dc + '35' }}><Text style={{ color: dc, fontSize: 10, fontWeight: '700' }}>{w.difficulty}</Text></View>}
                  {w.durationMin && <View style={{ backgroundColor: '#222', borderRadius: 999, paddingHorizontal: 8, paddingVertical: 2, borderWidth: 1, borderColor: '#333', flexDirection: 'row', alignItems: 'center', gap: 4 }}><Feather name="clock" size={10} color="#777" /><Text style={{ color: '#AAAAAA', fontSize: 10 }}>{w.durationMin}min</Text></View>}
                </View>
                {/* Bookmark */}
          ...
          {!wLoad && !wErr && wTotal > 1 && (
            <View style={{ flexDirection: 'row', justifyContent: 'center', gap: 12, marginVertical: 16 }}>
              <Pressable onPress={() => setWPage(p => Math.max(1, p - 1))} disabled={wPage <= 1}
                style={{ paddingHorizontal: 16, paddingVertical: 9, borderRadius: 999, backgroundColor: '#1A1A1A', borderWidth: 1, borderColor: '#333', opacity: wPage <= 1 ? 0.4 : 1 }}>
              ...
              <Pressable onPress={() => setWPage(p => Math.min(wTotal, p + 1))} disabled={wPage >= wTotal}
                style={{ paddingHorizontal: 16, paddingVertical: 9, borderRadius: 999, backgroundColor: '#1A1A1A', borderWidth: 1, borderColor: '#333', opacity: wPage >= wTotal ? 0.4 : 1 }}>
      ...
      {tab === 'exercises' && (
        <>
          <View className="px-5 pb-3">
            ...
            {/* Search */}
            <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#1A1A1A', borderRadius: 16, borderWidth: 1, borderColor: '#333', paddingHorizontal: 12, height: 46 }}>
                ...
                <View key={ex.id} style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#1A1A1A', borderRadius: 18, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: '#222' }}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <View style={{ flexDirection: 'row', gap: 6, marginTop: 6 }}>
                      {ex.category && <View style={{ backgroundColor: '#222', borderRadius: 999, paddingHorizontal: 8, paddingVertical: 2, borderWidth: 1, borderColor: '#333' }}><Text style={{ color: '#AAAAAA', fontSize: 10, fontWeight: '600' }}>{ex.category}</Text></View>}
                  ...
                  <View style={{ width: 34, height: 34, borderRadius: 17, backgroundColor: 'rgba(170,255,0,0.10)', borderWidth: 1, borderColor: 'rgba(170,255,0,0.25)', alignItems: 'center', justifyContent: 'center' }}>
                    <Feather name="play" size={14} color="#AAFF00" />
                  </View>
                </View>
              );
            })}
            ...
            {!exLoad && !exErr && exTotal > 1 && (
              <View style={{ flexDirection: 'row', justifyContent: 'center', gap: 12, marginVertical: 16 }}>
                <Pressable onPress={() => setExPage(p => Math.max(1, p - 1))} disabled={exPage <= 1}
                  style={{ paddingHorizontal: 16, paddingVertical: 9, borderRadius: 999, backgroundColor: '#1A1A1A', borderWidth: 1, borderColor: '#333', opacity: exPage <= 1 ? 0.4 : 1 }}>
                  <Text style={{ color: '#AAFF00', fontWeight: '700' }}>Prev</Text>
                </Pressable>
                <View style={{ backgroundColor: '#AAFF00', borderRadius: 999, paddingHorizontal: 16, paddingVertical: 9 }}>
                ...
                <Pressable onPress={() => setExPage(p => Math.min(exTotal, p + 1))} disabled={exPage >= exTotal}
                  style={{ paddingHorizontal: 16, paddingVertical: 9, borderRadius: 999, backgroundColor: '#1A1A1A', borderWidth: 1, borderColor: '#333', opacity: exPage >= exTotal ? 0.4 : 1 }}>
                  <Text style={{ color: '#AAFF00', fontWeight: '700' }}>Next</Text>
                </Pressable>
              </View>
            )}
