...
function Skeleton() {
  ...
  return <>{[0,1,2].map(k => <Animated.View key={k} style={{ height: 110, borderRadius: 20, backgroundColor: '#1A1A1A', marginBottom: 12, opacity: op }} />)}</>;
}
    ...
    <View className="flex-1 bg-bg" style={{ paddingTop: Platform.OS === 'android' ? (StatusBar.currentHeight ?? 24) : 0 }}>
      ...
      <ScrollView className="flex-1 px-5" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 48 }}>
        ...
        {!isLoading && !isError && items.length === 0 && (
          <View className="items-center pt-16">
            <View style={{ width: 68, height: 68, borderRadius: 34, backgroundColor: '#1A1A1A', borderWidth: 1, borderColor: '#222', alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
          ...
          return (
            <Pressable key={plan.id}
              ...
              <View style={{ flexDirection: 'row', gap: 6 }}>
                {plan.goal && <View style={{ backgroundColor: gc + '15', borderRadius: 999, paddingHorizontal: 8, paddingVertical: 2, borderWidth: 1, borderColor: gc + '35' }}><Text style={{ color: gc, fontSize: 10, fontWeight: '700' }}>{plan.goal}</Text></View>}
                {plan.difficulty && <View style={{ backgroundColor: dc + '15', borderRadius: 999, paddingHorizontal: 8, paddingVertical: 2, borderWidth: 1, borderColor: dc + '35' }}><Text style={{ color: dc, fontSize: 10, fontWeight: '700' }}>{plan.difficulty}</Text></View>}
                <View style={{ backgroundColor: plan.isActive ? 'rgba(170,255,0,0.10)' : '#222', borderRadius: 999, paddingHorizontal: 8, paddingVertical: 2, borderWidth: 1, borderColor: plan.isActive ? 'rgba(170,255,0,0.25)' : '#333' }}>
                  <Text style={{ color: plan.isActive ? '#AAFF00' : '#555', fontSize: 10, fontWeight: '600' }}>{plan.isActive ? 'Active' : 'Inactive'}</Text>
                </View>
              </View>
            </Pressable>
          );
