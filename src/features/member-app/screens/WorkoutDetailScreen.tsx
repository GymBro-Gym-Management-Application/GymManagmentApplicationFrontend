...
export default function WorkoutDetailScreen({ workoutId, userId, onBack }: Props) {
  ...
  return (
    <View className="flex-1 bg-bg" style={{ paddingTop: Platform.OS === 'android' ? (StatusBar.currentHeight ?? 24) : 0 }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
          ...
          <ImageBackground source={IMG_HERO} style={{ flex: 1 }} resizeMode="cover">
            <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.55)' }}>
              ...
              <View style={{ position: 'absolute', bottom: 16, alignSelf: 'center', backgroundColor: 'rgba(0,0,0,0.55)', borderRadius: 999, paddingHorizontal: 16, paddingVertical: 8, flexDirection: 'row', alignItems: 'center', gap: 8, borderWidth: 1, borderColor: 'rgba(255,255,255,0.18)' }}>
                <Feather name="clock" size={14} color="#AAFF00" />
        ...
        <View style={{ paddingHorizontal: 20 }}>
          ...
          {workout.exercises.map((ex, ei) => {
            ...
            return (
              <View key={ex.exerciseId} style={{ backgroundColor: '#1A1A1A', borderRadius: 20, padding: 16, marginBottom: 14, borderWidth: 1, borderColor: '#222' }}>
                {/* Exercise header */}
                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 14 }}>
                  <View style={{ width: 32, height: 32, borderRadius: 10, backgroundColor: '#222', alignItems: 'center', justifyContent: 'center', marginRight: 12 }}>
                    <Text style={{ color: '#AAFF00', fontSize: 13, fontWeight: '900' }}>{ei + 1}</Text>
                  </View>
                  <View style={{ flex: 1 }}>
                ...
                <View style={{ flexDirection: 'row', marginBottom: 8 }}>
                  <Text style={{ width: 36, color: '#555', fontSize: 10, fontWeight: '600' }}>SET</Text>
                  <Text style={{ flex: 1, color: '#555', fontSize: 10, fontWeight: '600', textAlign: 'center' }}>REPS</Text>
                  <Text style={{ flex: 1, color: '#555', fontSize: 10, fontWeight: '600', textAlign: 'center' }}>KG</Text>
                  <Text style={{ flex: 1, color: '#555', fontSize: 10, fontWeight: '600', textAlign: 'center' }}>RPE</Text>
                  <Text style={{ width: 36, color: '#555', fontSize: 10, fontWeight: '600', textAlign: 'center' }}>✓</Text>
                </View>
                ...
                {rows.map((row, ri) => (
                  <View key={ri} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                    ...
                    {(['reps', 'weight', 'rpe'] as const).map(field => (
                      <TextInput key={field}
                        style={{ flex: 1, backgroundColor: '#222', borderRadius: 10, height: 36, textAlign: 'center', color: row.done ? '#AAFF00' : '#FFFFFF', fontSize: 13, fontWeight: '600', marginHorizontal: 3, borderWidth: 1, borderColor: row.done ? 'rgba(170,255,0,0.30)' : '#333' }}
                        value={row[field]}
                    ...
                    <Pressable onPress={() => updateSet(ex.exerciseId, ri, 'done', !row.done)}
                      style={{ width: 36, height: 36, borderRadius: 10, backgroundColor: row.done ? '#AAFF00' : '#222', borderWidth: 1, borderColor: row.done ? '#AAFF00' : '#333', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <TextInput
            style={{ backgroundColor: '#1A1A1A', borderRadius: 16, borderWidth: 1, borderColor: '#333', color: '#FFFFFF', padding: 14, fontSize: 14, height: 80, textAlignVertical: 'top', marginBottom: 14 }}
          ...
          <TextInput
            style={{ backgroundColor: '#1A1A1A', borderRadius: 16, borderWidth: 1, borderColor: '#333', color: '#FFFFFF', padding: 14, fontSize: 14, marginBottom: 20 }}
      ...
      {/* ── Sticky bottom CTA ── */}
      <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: 20, paddingBottom: Platform.OS === 'ios' ? 34 : 20, backgroundColor: '#0D0D0D', borderTopWidth: 1, borderTopColor: '#1A1A1A' }}>
        <Pressable onPress={handleComplete} disabled={isPending}
          style={{ height: 56, borderRadius: 999, backgroundColor: isPending ? '#88CC00' : '#AAFF00', alignItems: 'center', justifyContent: 'center' }}>
          {isPending
