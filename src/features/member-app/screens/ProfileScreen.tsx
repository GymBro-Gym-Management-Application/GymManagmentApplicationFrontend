...
function Field({ label, value, onChange, placeholder, keyboardType, editable = true }: {
  ...
  return (
    <View style={{ marginBottom: 14 }}>
      ...
      <TextInput
        style={{
          backgroundColor: editable ? '#222222' : '#1A1A1A', borderRadius: 14,
          borderWidth: 1, borderColor: editable ? '#333333' : '#1E1E1E',
...
function TimelineRow({ event }: { event: { eventType: string; description: string; occurredAt: string } }) {
  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'flex-start', marginBottom: 14 }}>
      <View style={{ width: 32, height: 32, borderRadius: 10, backgroundColor: 'rgba(170,255,0,0.10)', borderWidth: 1, borderColor: 'rgba(170,255,0,0.22)', alignItems: 'center', justifyContent: 'center', marginRight: 12, marginTop: 2 }}>
        <Feather name={icon} size={14} color="#AAFF00" />
      </View>
      <View style={{ flex: 1 }}>
    ...
    <View className="flex-1 bg-bg" style={{ paddingTop: Platform.OS === 'android' ? (StatusBar.currentHeight ?? 24) : 0 }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 60 }}>
        ...
        <View style={{ alignItems: 'center', paddingTop: 32, paddingBottom: 24, paddingHorizontal: 20 }}>
          ...
          {/* Status badge */}
          <View style={{ marginTop: 10, backgroundColor: profile?.status === 'Active' ? 'rgba(170,255,0,0.12)' : '#1A1A1A', borderWidth: 1, borderColor: profile?.status === 'Active' ? 'rgba(170,255,0,0.30)' : '#333', borderRadius: 999, paddingHorizontal: 12, paddingVertical: 5 }}>
            <Text style={{ color: profile?.status === 'Active' ? '#AAFF00' : '#AAAAAA', fontSize: 11, fontWeight: '700' }}>
