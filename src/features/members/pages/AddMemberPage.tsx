  ...
  return (
    <View className="flex-1 bg-bg" style={{ paddingTop: Platform.OS === 'android' ? (StatusBar.currentHeight ?? 24) : 0 }}>
      ...
      <ScrollView className="flex-1 px-5" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: 16, paddingBottom: 48 }}>
            ...
            return (
              <Pressable key={g.value} onPress={() => setGender(g.value)}
                style={{ flex: 1, paddingVertical: 12, borderRadius: 16, borderWidth: 1.5, alignItems: 'center',
                  backgroundColor: sel ? 'rgba(170,255,0,0.12)' : '#222222',
                  borderColor: sel ? 'rgba(170,255,0,0.50)' : '#333333' }}>
                <Text style={{ color: sel ? '#AAFF00' : '#AAAAAA', fontSize: 13, fontWeight: '700' }}>{g.label}</Text>
              </Pressable>
            );
          })}
        </View>
        ...
        {/* Info note */}
        <View style={{ flexDirection: 'row', alignItems: 'flex-start', backgroundColor: 'rgba(170,255,0,0.07)', borderWidth: 1, borderColor: 'rgba(170,255,0,0.20)', borderRadius: 14, padding: 12, marginBottom: 16, gap: 10 }}>
          <Feather name="info" size={15} color="#AAFF00" style={{ marginTop: 1 }} />
